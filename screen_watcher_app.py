...
st.set_page_config(page_title="Screen Watcher", page_icon="👁", layout="centered")
...
.metric-value { font-family: 'IBM Plex Mono', monospace; font-size: 28px; font-weight: 600; color: #e2e8f0; line-height: 1.1; }
.metric-label { font-size: 11px; color: #718096; text-transform: uppercase; letter-spacing: 0.08em; margin-top: 4px; }
...
div[data-testid="stButton"] > button:hover { border-color: #48bb78; color: #48bb78; background: #0f1f18; }
div[data-testid="stTextInput"] input {
    font-family: 'IBM Plex Mono', monospace; background: #161a22;
    border: 1px solid #2d3748; border-radius: 8px; color: #e2e8f0;
}
div[data-testid="stCheckbox"] label { color: #94a3b8; }
...
st.markdown(
    ...
    "Upload screenshots — OCR scans each one and alerts you when target text is found.</p>",
    unsafe_allow_html=True,
)

# ── Config (only editable when not watching) ──────────────
with st.expander("⚙️ Configuration", expanded=not st.session_state.watching):
    ...
    if not st.session_state.watching:
        ...
        st.session_state.case_sensitive = case_input
...
if st.session_state.watching:
    if st.session_state.last_result == "found":
        ...
        msg        = f'<span style="color:#f6ad55;font-weight:600;">"{target}"</span> was detected!'
    else:
        badge      = '<span class="badge badge-green">● WATCHING</span>'
        card_class = "status-card status-watching"
        msg        = f'Waiting for screenshots. Looking for: <span style="color:#e2e8f0;">"{target}"</span>'
else:
    badge      = '<span class="badge badge-gray">○ IDLE</span>'
    card_class = "status-card status-idle"
    msg        = 'Press <b style="color:#e2e8f0;">Start Watching</b> to begin.'

st.markdown(f'<div class="{card_class}">{badge}<div style="margin-top:10px;color:#94a3b8;">{msg}</div></div>',
            unsafe_allow_html=True)
...
if not st.session_state.watching:
    st.caption("Start watching first, then upload screenshots here.")
...
if uploaded_file is not None and st.session_state.watching:
    file_id = f"{uploaded_file.name}-{uploaded_file.size}"

    if file_id != st.session_state.last_file_id:
        # New image uploaded — scan it
        ...
        try:
            ...
            needle   = target if st.session_state.case_sensitive else target.lower()
            haystack = extracted if st.session_state.case_sensitive else extracted.lower()
            ...
            if found:
                ...
                st.session_state.log.append(("match",    ts, f'MATCH — "{target}" found'))
                st.balloons()
            else:
                st.session_state.last_result = "notfound"
                st.session_state.log.append(("notfound", ts, f'No match for "{target}"'))
...
# ── Last scan result ──────────────────────────────────────
if st.session_state.last_result == "found":
    st.success(f'✅ **"{target}"** found in the last scan!')
elif st.session_state.last_result == "notfound":
    st.info(f'🔍 **"{target}"** not found in the last scan.')
elif st.session_state.last_result == "error":
    st.error("An error occurred during the last scan. Check the log.")

if st.session_state.last_extracted:
    with st.expander("📄 Last extracted text (OCR output)"):
        st.code(st.session_state.last_extracted, language=None)
...
<div class="metric-row">
  <div class="metric-box"><div class="metric-value">{checks}</div><div class="metric-label">Scans</div></div>
  <div class="metric-box"><div class="metric-value" style="color:#f6ad55">{matches}</div><div class="metric-label">Matches</div></div>
  <div class="metric-box"><div class="metric-value">{rate}</div><div class="metric-label">Match rate</div></div>
</div>
""", unsafe_allow_html=True)
